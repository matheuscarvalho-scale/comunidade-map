...
import { CheckCircle2, Flame, Crown, Rocket, Star, Loader2, ArrowUp, Shield } from "lucide-react";
...
import { canUpgradeTo } from "@/hooks/usePlanUpgrade";
...
export default function Planos() {
  ...
  const hasActivePlan = isLoggedIn && subscription?.canAccess && currentPlan && PLAN_HIERARCHY.includes(currentPlan);
  ...
  const handleCheckout = (planId: string) => {
    ...
    const checkoutUrl = ASAAS_CHECKOUT_URLS[planId];
    if (checkoutUrl) {
  ...
  const getPlanStatus = (planId: string) => {
    if (!hasActivePlan) return "available";
    if (planId === currentPlan) return "current";
    if (canUpgradeTo(currentPlan, planId)) return "upgradeable";
    return "lower";
  };
  ...
  return (
    <div className="min-h-screen bg-background">
      <SEOHead
        ...
        description="Escolha o plano ideal para acelerar seus resultados no e-commerce com o MAP Acelera."
        canonical="/planos"
      />
      ...
      <header className="fixed top-0 left-0 right-0 z-50 border-b border-border/50 bg-background/80 backdrop-blur-lg">
        <div className="container mx-auto px-4 h-16 flex items-center justify-between">
          <button onClick={() => navigate(isLoggedIn ? "/" : "/comunidade-map")} className="flex items-center gap-2">
            <img src={isDarkMode ? logoMapDark : logoMapLight} alt="MAP Acelera - Comunidade de Empreendedores" className="h-8 w-auto" />
          </button>
          {isLoggedIn ? (
        ...
        <div className="container mx-auto max-w-3xl">
          <div className="inline-flex items-center gap-2 rounded-full bg-primary/10 px-4 py-2 mb-6">
            ...
            <span className="text-sm text-primary font-medium">
              {hasActivePlan ? "Gerencie seu plano" : "Escolha o plano ideal para você"}
            </span>
          </div>
          ...
          <p className="text-muted-foreground text-lg max-w-xl mx-auto">
            {hasActivePlan ? (
              isMaxPlan
                ...
                : "Faça upgrade para desbloquear mais benefícios. Seu parcelamento anterior continua normalmente."
            ) : (
              "Todos os planos incluem acesso anual completo à plataforma. Escolha o nível que mais combina com o momento do seu negócio."
            )}
          </p>
        </div>
      </section>
      ...
      {isLoggedIn && subLoading ? (
        ...
        /* Plans Grid */
        <section className="pb-20 px-4" aria-labelledby="planos-heading">
      ...
      <footer className="py-8 px-4 border-t border-border/50">
        <div className="container mx-auto text-center">
          <div className="flex items-center justify-center gap-2 mb-4">
            <img src={isDarkMode ? logoMapDark : logoMapLight} alt="MAP Acelera - Comunidade de Empreendedores" className="h-6 w-auto" />
            <Flame className="h-4 w-4 text-primary" />
          </div>
...
function PlanButton({
  ...
  status: "available" | "current" | "upgradeable" | "lower";
  planName: string;
  isLoading: boolean;
  loadingAny: boolean;
  highlight: boolean;
  ...
  // available (no plan / not logged in)
  return (
    <Button
      size="lg"
      disabled={loadingAny}
    ...
    >
      {isLoading ? (
        ...
        <>APLICAR PARA O {planName.toUpperCase()}</>
      )}
    </Button>
  );
}
