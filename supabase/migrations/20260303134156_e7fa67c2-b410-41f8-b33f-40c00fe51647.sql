...
-- RPC: Get engagement overview stats
...
DECLARE
  ...
  result jsonb;
BEGIN
  -- Total members with active subscription
  ...
  -- Inactive 20+ days (have subscription but no activity)
...
-- RPC: Get top pages by views (last 30 days)
...
-- RPC: Get formation completion rates
...
BEGIN
  ...
  SELECT 
    ...
    au.email::TEXT,
    p.avatar_url,
    latest.last_seen as last_activity,
    EXTRACT(DAY FROM now() - latest.last_seen)::INTEGER as days_inactive,
    p.subscription_plan
  FROM profiles p
  ...
  WHERE p.subscription_status = 'active'
    AND (latest.last_seen IS NULL OR latest.last_seen < now() - (inactive_days || ' days')::INTERVAL)
  ORDER BY latest.last_seen ASC NULLS FIRST
  LIMIT limit_count;
END;
$$;
