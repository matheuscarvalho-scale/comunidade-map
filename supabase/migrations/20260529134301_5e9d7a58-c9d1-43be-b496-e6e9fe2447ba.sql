...
CREATE OR REPLACE FUNCTION public.notify_new_content_webhook(_tipo text, _titulo text, _presenter text DEFAULT NULL)
RETURNS void
LANGUAGE plpgsql
SECURITY DEFINER
...
DECLARE
  ...
  _presenter text;
BEGIN
  SELECT is_active, is_coming_soon, presenter_name INTO _track
  ...
  IF COALESCE(_track.is_active, false) = true AND COALESCE(_track.is_coming_soon, false) = false THEN
    ...
    PERFORM public.notify_new_content_webhook('novo_conteudo', NEW.title, _presenter);
  END IF;
  RETURN NEW;
END;
$function$;
...
BEGIN
  IF TG_OP = 'INSERT' THEN
    IF COALESCE(NEW.is_active, false) = true AND COALESCE(NEW.is_coming_soon, false) = false THEN
      PERFORM public.notify_new_content_webhook('nova_trilha', NEW.title, NEW.presenter_name);
    END IF;
...
BEGIN
  IF TG_OP = 'INSERT' THEN
    IF COALESCE(NEW.is_published, false) = true AND COALESCE(NEW.is_coming_soon, false) = false THEN
      PERFORM public.notify_new_content_webhook('nova_formacao', NEW.title, NEW.presenter_name);
    END IF;
  ELSIF TG_OP = 'UPDATE' THEN
