...
AS $$
  SELECT 
    ...
    COALESCE(pe.amount, 0)::NUMERIC as valor_plano,
    COALESCE(SUM(cu.cashback_amount) FILTER (WHERE cu.status = 'approved'), 0)::NUMERIC as cashback_gerado,
    (COALESCE(pe.amount, 0) - COALESCE(SUM(cu.cashback_amount) FILTER (WHERE cu.status = 'approved'), 0))::NUMERIC as saldo_a_pagar
  FROM profiles p
